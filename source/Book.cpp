 ...
 * File:   Book.cpp
 *
 * Polyglot opening book reader implementation.
 * Reference: https://allstar.jhuapl.edu/repo/p4/amd64/polyglot/doc/book_format.html
 *
 * The Polyglot random number table below is taken from the Polyglot source code
...
// ============================================================================
// Published Polyglot Zobrist random numbers (781 entries).
// Layout: RandomPiece[768], RandomCastle[4], RandomEnPassant[8], RandomTurn[1]
// ============================================================================
static const U64 Random64[781] = {
...
// Offsets into Random64 for each sub-array
static constexpr int RANDOM_PIECE = 0;        // 768 entries (12 pieces * 64 squares)
static constexpr int RANDOM_CASTLE = 768;     // 4 entries
static constexpr int RANDOM_ENPASSANT = 772;  // 8 entries
static constexpr int RANDOM_TURN = 780;       // 1 entry

// ============================================================================
// Map engine piece type to Polyglot "kind_of_piece" index.
// Polyglot: BlackPawn=0, WhitePawn=1, BlackKnight=2, WhiteKnight=3, ...
// Engine:   WHITE_PAWN=2, BLACK_PAWN=3, WHITE_KNIGHT=4, BLACK_KNIGHT=5, ...
// ============================================================================
static int engine_piece_to_polyglot(U8 piece)
{
    // Engine pieces are: 2=WP, 3=BP, 4=WN, 5=BN, 6=WB, 7=BB, 8=WR, 9=BR, 10=WQ, 11=BQ, 12=WK, 13=BK
    // Polyglot kinds:    1=WP, 0=BP, 3=WN, 2=BN, 5=WB, 4=BB, 7=WR, 6=BR, 9=WQ, 8=BQ, 11=WK, 10=BK
    // Pattern: for white pieces (even engine index), polyglot = engine - 2 + 1 (odd)
    //          for black pieces (odd engine index),  polyglot = engine - 2 - 1 (even)
    // Simplified: swap the color bit
    static const int mapping[] = {
        -1, -1,  // 0=EMPTY, 1=EMPTY
        1,  0,   // 2=WHITE_PAWN  -> 1, 3=BLACK_PAWN  -> 0
        3,  2,   // 4=WHITE_KNIGHT-> 3, 5=BLACK_KNIGHT-> 2
        5,  4,   // 6=WHITE_BISHOP-> 5, 7=BLACK_BISHOP-> 4
        7,  6,   // 8=WHITE_ROOK  -> 7, 9=BLACK_ROOK  -> 6
        9,  8,   // 10=WHITE_QUEEN-> 9, 11=BLACK_QUEEN-> 8
        11, 10,  // 12=WHITE_KING ->11, 13=BLACK_KING ->10
    };
    return mapping[piece];
}

// ============================================================================
// polyglot_hash: compute the Polyglot-compatible Zobrist key from board state.
// Uses the published Polyglot random numbers, NOT the engine's internal keys.
// ============================================================================
U64 Book::polyglot_hash(const Board& board)
{
    U64 key = 0;
    ...
    for (int sq = 0; sq < 64; ++sq)
    {
        ...
        if (piece != EMPTY)
        {
            int pg_kind = engine_piece_to_polyglot(piece);
            int file = sq & 7;
            int row = sq >> 3;
            // Polyglot offset: 64 * kind_of_piece + 8 * row + file
            key ^= Random64[RANDOM_PIECE + 64 * pg_kind + 8 * row + file];
        }
    }
    ...
    // 3. En passant key — only if there is actually a pawn that can capture.
    // Polyglot spec: only set if opponent did a double push AND there is a
    // pawn of the side to move adjacent to the pushed pawn.
    U8 ep = board.ep_square();
    if (ep != NULL_SQUARE)
    {
        int ep_file = ep & 7;
        int ep_row = ep >> 3;
        bool has_ep_capture = false;

        if (board.side_to_move() == WHITE)
        {
            // White pawns on row 4 (rank 5) adjacent to ep file can capture
            int pawn_row = 4;  // row index of rank 5 (0-indexed)
            if (ep_row == 5)   // ep square is on rank 6
            {
                if (ep_file > 0 && board[pawn_row * 8 + ep_file - 1] == WHITE_PAWN)
                    has_ep_capture = true;
                if (ep_file < 7 && board[pawn_row * 8 + ep_file + 1] == WHITE_PAWN)
                    has_ep_capture = true;
            }
        }
        else
        {
            // Black pawns on row 3 (rank 4) adjacent to ep file can capture
            int pawn_row = 3;  // row index of rank 4 (0-indexed)
            if (ep_row == 2)   // ep square is on rank 3
            {
                if (ep_file > 0 && board[pawn_row * 8 + ep_file - 1] == BLACK_PAWN)
                    has_ep_capture = true;
                if (ep_file < 7 && board[pawn_row * 8 + ep_file + 1] == BLACK_PAWN)
                    has_ep_capture = true;
            }
        }
        ...
        if (has_ep_capture)
        {
            key ^= Random64[RANDOM_ENPASSANT + ep_file];
        }
    }

    // 4. Turn key — XOR if white to move
    if (board.side_to_move() == WHITE)
    {
...
// ============================================================================
// Read a big-endian unsigned integer from a byte buffer.
// ============================================================================
static U64 read_u64_be(const unsigned char* p)
{
...
{
    return static_cast<U16>((static_cast<unsigned>(p[0]) << 8) | p[1]);
}
...
// ============================================================================
// open: Load a Polyglot .bin book file into memory.
// ============================================================================
bool Book::open(const std::string& path)
{
    ...
    auto num_entries = static_cast<size_t>(size) / 16;
    ...
    for (size_t i = 0; i < num_entries; ++i)
    {
        ...
        entries_[i].key = read_u64_be(p);
        entries_[i].move = read_u16_be(p + 8);
        entries_[i].weight = read_u16_be(p + 10);
        entries_[i].learn = read_u32_be(p + 12);
    }
...
// ============================================================================
// find_entries: Binary search for all entries matching the given key.
// The file is sorted by key, so we find the first match and scan forward.
// ============================================================================
std::vector<Book::PolyglotEntry> Book::find_entries(U64 key) const
{
    ...
    while (lo < entries_.size() && entries_[lo].key == key)
    {
        if (entries_[lo].weight > 0)
            result.push_back(entries_[lo]);
        ++lo;
    }
...
// ============================================================================
// polyglot_move_to_engine_move: Convert a Polyglot move encoding to Move_t.
//
// Polyglot move bits:
//   0-2:   to_file
//   3-5:   to_row
...
//
// Polyglot castling: king moves to ROOK square:
//   e1h1 (white king-side), e1a1 (white queen-side)
//   e8h8 (black king-side), e8a8 (black queen-side)
// ============================================================================
Move_t Book::polyglot_move_to_engine_move(U16 pg_move, const Board& board)
{
    int to_file = (pg_move >> 0) & 7;
    int to_row = (pg_move >> 3) & 7;
    int from_file = (pg_move >> 6) & 7;
    int from_row = (pg_move >> 9) & 7;
    ...
    // Detect castling: king moving to rook square in Polyglot encoding
    if ((piece & ~1U) == KING)
    {
        // White king on e1
        if (from_sq == E1 && to_sq == H1)
            return build_castle(KING_CASTLE);
        if (from_sq == E1 && to_sq == A1)
            return build_castle(QUEEN_CASTLE);
        // Black king on e8
        if (from_sq == E8 && to_sq == H8)
            return build_castle(KING_CASTLE);
        if (from_sq == E8 && to_sq == A8)
            return build_castle(QUEEN_CASTLE);
    }

    // Map Polyglot promotion piece to engine piece type
    // Polyglot: 1=knight, 2=bishop, 3=rook, 4=queen
    auto promo_piece = [&]() -> U8
    {
        U8 color_offset = (board.side_to_move() == WHITE) ? 0 : 1;
        switch (promo)
        {
            case 1:
                return static_cast<U8>(WHITE_KNIGHT + color_offset);
            case 2:
                return static_cast<U8>(WHITE_BISHOP + color_offset);
            case 3:
                return static_cast<U8>(WHITE_ROOK + color_offset);
            case 4:
                return static_cast<U8>(WHITE_QUEEN + color_offset);
            default:
                return EMPTY;
        }
    };

    U8 from = static_cast<U8>(from_sq);
    U8 to = static_cast<U8>(to_sq);
    U8 captured = board[to_sq];

    // En passant capture
    if ((piece & ~1U) == PAWN && to == board.ep_square() && board.ep_square() != NULL_SQUARE)
    {
        U8 ep_captured = (board.side_to_move() == WHITE) ? BLACK_PAWN : WHITE_PAWN;
        return build_ep_capture(from, to, ep_captured);
    }
    ...
    if (promo != 0)
    {
        U8 pp = promo_piece();
        if (captured != EMPTY)
            return build_capture_promotion(from, to, captured, pp);
        return build_promotion(from, to, pp);
    }

    // Capture
    if (captured != EMPTY)
        return build_capture(from, to, captured);

    // Pawn double push
    if ((piece & ~1U) == PAWN && std::abs(to_row - from_row) == 2)
        return build_pawn_double_push(from, to);

    // Normal move
    return build_move(from, to);
}

// ============================================================================
// has_move: Check if the book has any moves for the current board position.
// ============================================================================
bool Book::has_move(const Board& board) const
{
    U64 key = polyglot_hash(board);
    auto entries = find_entries(key);
    return !entries.empty();
}

// ============================================================================
// get_move: Return a weighted-random book move for the current position.
// Returns Move(0) if no book move is found.
// ============================================================================
Move_t Book::get_move(const Board& board) const
{
    U64 key = polyglot_hash(board);
    auto entries = find_entries(key);
    ...
    for (const auto& e : entries)
    {
        cumulative += e.weight;
        if (pick < cumulative)
            return polyglot_move_to_engine_move(e.move, board);
    }

    // Fallback (should not reach here)
    return polyglot_move_to_engine_move(entries.back().move, board);
}
